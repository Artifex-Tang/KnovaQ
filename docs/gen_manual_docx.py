...
OUT = ROOT / "output" / "DARPA智能问答服务工具-软件用户手册-v5.docx"
...
def build():
    ...
    t2 = doc.tables[2]
    fill_cell(t2.rows[1].cells[0], "V1.0")
    fill_cell(t2.rows[1].cells[1], "2026-06-04")
    fill_cell(t2.rows[1].cells[2], "初始版本")
    ...
    add_list_item(doc, "c）软件版本号：V1.0。")
    ...
    add_para(doc, "本章向用户提供使用DARPA智能问答服务工具的完整操作过程。")

    add_heading(doc, "能力", 2)
    add_para(doc, "系统三大核心模块的协作关系：用户提问后，M3交互式提示词工程模块构建提示请求，调用M2 RAG检索增强模块从M1外挂知识库模块中检索知识片段，最终由本地大语言模型（智谱GLM-9B）生成精准答案并返回用户。")
    add_para(doc, "M1外挂知识库模块提供知识库全生命周期管理能力：创建/删除/修改知识库，上传PDF/Word/Excel/TXT/图片等多格式军事文档，自动进行文档深度解析、智能语义化分块、元数据标注与过滤、文档解析状态监控。")
    ...
    add_list_item(doc, "c）界面约定：菜单项统一位于页面顶部导航栏或左侧菜单栏；操作按钮统一位于对应功能区域上方或右侧；列表页面支持翻页浏览，每页默认显示20条记录；必填字段以红色星号（*）标识；")
    add_list_item(doc, "d）操作术语：\"点击\"为鼠标左键单击；\"输入\"为在文本框中键入内容；\"选择\"为在下拉列表中选取选项；\"勾选\"为在复选框中打勾。")

    # ── 4.3 处理过程 ──
    add_heading(doc, "处理过程", 2)

    # ── 4.3.1-4.3.8: NEW sections (app pages) ──
